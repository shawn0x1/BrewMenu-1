...
class Rect():
	def __init__(self, startx, starty, endx, endy):
		self.tl = (startx, starty)	# Top-left corner
		self.tr = (endx, starty)	# Top-right corner
		self.br = (endx, endy)	# Bottom-right corner
		self.bl = (startx, endy)	# Bottom-left corner
		self.outline = [self.tl, self.tr, self.br, self.bl]
...
class Menu():
	...
	# def make_cols(self):
		...
		for n in range(self.ncols):
			...
			col_title = list(self.menudict.keys())[n]
			## Split each key-value entry in menudict into its own dict --> {'Title': ['Values',...]}
			col_text = {col_title : self.menudict[col_title]}
			col = Rect(last_col_x, self.sy, last_col_x+per_col_w, self.ey)#, col_text)
			self.cols.append(col)
			last_col_x += col.tl[0] + self.thick
...
def demo(screen):
	...
	# menu = Rect(2, 4, screen.width-2, screen.height-4)
	# inner = Rect(4,8, screen.width-4, screen.height-8)
	...
	while True:
		...
		menu.draw()
		# poly = []
		# poly.append(menu.outline)
		# poly.append(inner.outline)
		# screen.fill_polygon(poly, colour=textcolor)
